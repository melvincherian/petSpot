...
class AccesoriesearchBloc extends Bloc<AccesoriesearchEvent, AccesoriesearchState> {
...
final List<ProductAccessoriesModel>_allAccessories=[];
  ...
  try {
    ...
    on<SearchAccessories>((event, emit) {
      final searchaccessories=_allAccessories.where((accessories)=>accessories.accesoryname.toLowerCase().contains(event.query.toLowerCase())).toList();
